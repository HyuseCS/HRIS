...
| `tests/unit/payroll-statutory-config.test.ts:120` (live table `ceiling: Infinity` vs jsonb payload `ceiling: null`) and `:137` (default config, already `ceiling: null` via `*ToWire`, vs the payload) both compare equal | Fully-Automated | AC-S7a.4 |
...
| R13 | S7a's field-by-field compare hits `Infinity` (live table ceiling) against `null` (jsonb payload) at `payroll-statutory-config.test.ts:120`, and the default config — already `null` via `*ToWire`, not `Infinity` — against the payload at `:137`, and either emits a false "table changed" line — F5's own bug, reintroduced by F5's fix | S7a step 2b makes open-ceiling normalisation binding; the `payroll-statutory-config.test.ts` cases at `:120` and `:137` are its negative controls (`AC-S7a.4`). |
