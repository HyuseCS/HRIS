...
| 12 | Deactivate a login | A | **DONE** (S5), **amended 11-09-26 (remediation A, S8)**. As shipped in S5 it branched on `u.isActive` with `ConfirmButton` on deactivate only, the activate branch keeping its plain form and `setActiveGuard`. The owner reversed that call: **both** directions now confirm, each naming its own consequence. The per-row `setActiveGuards` / `setActiveGuard` bindings are removed — `ConfirmButton` owns the #108 single-submit guard for both branches. |
