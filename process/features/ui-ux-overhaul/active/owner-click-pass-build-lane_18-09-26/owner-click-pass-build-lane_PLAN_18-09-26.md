...
**Serial mode alone does not close the race.** `playwright.config.ts:20` sets `fullyParallel: true` and `:23` sets `workers: process.env.CI ? 1 : undefined`, so serial ordering applies WITHIN this file only — locally `pagination.spec.ts` can still inject its 25 rows into the same org while this spec walks pages. So the three unfiltered multi-page walks (`table is bounded`, `counter tells the truth`, `filter applies across pages`) MUST search the `Zzorgtest` marker first and page within that filtered set, mirroring what `pagination.spec.ts` already does with its surname filter. Reading M from the page is necessary but not sufficient on its own. Running the suite with `CI=1` (`workers: 1`) is the alternative.
...
Mitigation, and it is the only one: **the full `bun run test:e2e` after Commit 4 is the gate that catches this**, not any per-lane run. A lane-local green on `page-header-helptip` before N4 lands does not carry. If the anchoring assertions prove brittle under the narrower cluster, re-point that spec at `/inventory` (`inventory/+page.svelte:33-36`, a bare child of `space-y-6`). Do NOT use `/complaints`: as an HR user it already renders a title-row `{@render actions()}` cluster (`complaints/+page.svelte:77-87`), so it is not a control-free reference page.
...
- The **page change** half is not, and **the owner has taken it out of this lane.** Paging goes through `Pagination.svelte`'s `<a>` elements; on navigation SvelteKit resets focus to `<body>` by default. Fixing it means editing **`Pagination.svelte`, which is imported by 19 files across 17 routes** (15 route pages, plus `/attendance` via `AttendanceHrGrid.svelte`, `AttendanceSelfView.svelte` and `TeamMatrix.svelte`, plus `/timesheets` via `TimesheetListTab.svelte`). None of those is in this plan's blast radius.
...
`process/features/ui-ux-overhaul/backlog/pagination-drops-keyboard-focus_NOTE_18-09-26.md` (ALREADY ON DISK — do not create a new note) — records that `Pagination.svelte` drops focus to `<body>` on every page change across every paginated route, names `data-sveltekit-keepfocus` on its two `<a>` elements as a **partial** candidate fix, records that the fix is **incomplete on its own** because `← Previous` on page 1 (`:40`) and `Next →` on the last page (`:55`) are `<span>`s with no anchor to keep focus on, and notes that the change is app-wide — `Pagination.svelte` is imported by **19 files across 17 routes** — so it alters focus behaviour everywhere at once and needs its own review.
...
- **VERIFIED** only when, in addition: both negative controls in [Negative controls](#negative-controls) have been run and recorded as going **red** on mutation and green on revert; the N7-AC8 contrast probe numbers are recorded in the phase report for both themes; the owner has confirmed the three Hybrid focus-ring/screen-reader clauses (N1-AC8, N4-AC5, N6-AC10) on a live page in light and dark; and the N6-AC10 backlog note exists at `process/features/ui-ux-overhaul/backlog/pagination-drops-keyboard-focus_NOTE_18-09-26.md` — it already does, so this clause is satisfied on arrival and nothing new is written.
...
supersedes: 2026-09-18 (outer-pvl) — second outer-PVL pass after the plan was revised to clear FAIL-1/2/3; this contract has current evidence

Parallel strategy: sequential (single validate-agent, batched read-only probes)
Rationale: 2/7 signals (S5 the caller asked for per-claim verification, S7 12 files in the blast radius). MEDIUM band. The Layer 1 and Layer 2 roles were run in-process as batched read-only Bash probes plus two executable checks. Every finding below carries file:line evidence or a recorded command output; none is inferred.

**Every one of the three prior FAILs is CONFIRMED FIXED against source.** Two of the three fixes were verified by *executing* the predicate, not by reading it.

### Test gates

| criterion id | behavior | strategy | proving test | gap-resolution |
|---|---|---|---|---|
| N7-AC1..AC4 | PageHeader renders description as HelpTip; props unchanged; diff scope is 3 files | Fully-Automated | `bun run test -- page-header-helptip` | A |
| N7-AC5 | hover AND focus each reveal, asserted separately | Fully-Automated | e2e `page-header-helptip` › `hover reveals`, `focus reveals` (`toHaveCSS('opacity','1')`) | A |
...
| N7-AC7 | exactly one `?` from the description path per page | Fully-Automated | e2e › `exactly one description-path help control per page` (4-page table) + unit › opening-tag-scoped invariant scan | A |
...
| N7-AC10 | `/complaints/[id]` keeps an identifying title | Fully-Automated | e2e › `complaints detail keeps an identifying title` | A |
| N1-AC1..AC5 | two-state switch, one-click flip, round trip, grid landing, `?view=` contract | Fully-Automated | e2e `attendance-view-switch` (5 tests) | A |
| N1-AC6 | per-day controls collapse to one row (alignment AND separation) | Fully-Automated | e2e › `per day controls are one row` | A |
| N1-AC7 | employee-view controls untouched | Fully-Automated | e2e › `employee view controls untouched` | A |
| N1-AC8 | tab order, accessible names, focus ring | Hybrid | e2e › `keyboard order and names` + owner light/dark check | A |
| N1-AC9 | employee role sees no team switch | Fully-Automated | corrected `employee-view-only.spec.ts:170` + Negative control 1 | A |
| N4-AC1, AC2 | action on the title row; no standalone action row | Fully-Automated | e2e `separations` › `action sits on the title row`, `no standalone action row` | A |
| N4-AC3 | the moved button still opens the create dialog | Fully-Automated | e2e `separations` › `the button still opens the create dialog` | A |
| N4-AC4 | `PageHeader` gains no `actions` prop | Fully-Automated | unit `page-header-helptip` › props unchanged | A |
| N4-AC5 | visible focus ring on the title-row action | Hybrid | e2e › `title row a11y` + owner light/dark check | A |
| N6-AC1..AC4, AC7, AC9 | bounded table, paging, filters in the URL, filter spans pages, full positions catalog, no control when it fits | Fully-Automated | e2e `settings-org-assignments` (6 tests) | A |
| N6-AC5, AC6, AC8 | filter resets to page 1; `empPage` never collides with `page`; counter inputs | Fully-Automated | unit `settings-org-load` (8 tests) + e2e › `filter resets page, paging keeps filter`, `counter tells the truth` | A |
| N6-AC10 (filter half, labels, link names, announcement) | focus survives a filter change; controls are named and keyboard-operable | Fully-Automated | e2e › `filters and paging a11y` | A |
| N6-AC10 (page-change focus clause) | focus not lost to `<body>` after a page change | — named residual — | none in this lane | D — DEFERRED by explicit owner decision to the existing backlog note `pagination-drops-keyboard-focus_NOTE_18-09-26.md` |
...
- N1 / `AttendanceHrGrid`: [Fully-automated: `bun run test:e2e -- attendance-view-switch`, `bun run test:e2e -- employee-view-only`] | [hybrid: owner focus-ring check in light and dark]
- N4 / `separations/+page.svelte`: [Fully-automated: `bun run test:e2e -- separations`] | [hybrid: owner focus-ring check]
- N6 / `settings/org`: [Fully-automated: `bun run test -- settings-org-load`, `bun run test:e2e -- settings-org-assignments`] | [known-gap: page-change keyboard focus — documented, owner-deferred]

**Gate-command validity confirmed (executed this session).** The backlog note `e2e-spec-filter-silently-ignored_NOTE_10-09-26.md` claims scoped e2e runs silently run the whole suite. That was a **pnpm-era** defect. Under bun the filter works: `bun run test:e2e -- separations --list` expands to `dotenv -e .env.dev -- playwright test separations --list` and reports `Total: 6 tests in 1 file`. `bun run test -- badge-class-literals` likewise expands to `vitest run badge-class-literals` and ran 1 file / 13 tests. bun strips the `--`; pnpm forwarded it. Every targeted command in [Gates](#gates) is therefore valid. The note is stale and should be marked superseded — out of this lane's scope.

### The three prior FAILs — re-verified against source

**FIX-1 (was FAIL-1) — the badge-HelpTip evidence table. CONFIRMED FIXED.**

Every cited line resolves:

- `src/routes/(app)/leave/balances/+page.svelte:31` `<PageHeader title="Leave Balances">`, badge `HelpTip label="About leave balances"` at `:33`. No `description`. ✓
- `src/routes/(app)/settings/roles/+page.svelte:166` `<PageHeader title="Roles & Permissions">`, badge `HelpTip label="About roles and permissions"` at `:168`. No `description`. ✓
- `src/lib/components/attendance/TeamMatrix.svelte` — `grep -c PageHeader` returns **0**. Its `HelpTip label="About team attendance"` is at `:75` inside `{#snippet toolbar()}` (opened `:70`), passed to `<Container tone="card" flush {toolbar} …>` at `:111`. ✓ Not a `PageHeader` badge.
- `src/routes/(app)/dashboard/+page.svelte:138` is a bare `<PageHeader title="Dashboard" />`; its only `HelpTip` is at `:253`, inside the Upcoming Regularizations popover. ✓

**Independently re-derived, not taken from the plan.** A script walked every `.svelte` file under `src/`, matched `<PageHeader\b[^>]*>` opening tags, and cross-checked `{#snippet badge()}` bodies:

- 33 `PageHeader` opening tags carry `description=`, across **31 files** (62 `<PageHeader` occurrences total in `src/`).
- Files passing BOTH a `description` and a `badge` snippet: exactly **three** — `complaints/[id]`, `performance/reviews/[id]`, `separations/[id]`. All three badges are `<Badge status=… domain=… />` status pills. **Zero** of them contain a `HelpTip`.
- Therefore no page today renders two `?`, and `{#if description}` alone yields the correct count on every page. The plan's conclusion and its evidence both hold.

**FIX-2 (was FAIL-2) — N7-AC7 rewritten. CONFIRMED FIXED.** Each expected number verified:

| Page | description-path | other `About …` | total | verified against |
|---|---|---|---|---|
| `/settings/roles` | 0 | 1 | 1 | `settings/roles/+page.svelte:166` (no description), `:168` |
| `/leave/balances` | 0 | 1 | 1 | `leave/balances/+page.svelte:31` (no description), `:33` |
| `/attendance` | 1 | 1 | 2 | `AttendanceHrGrid.svelte:209` description; `TeamMatrix.svelte:75` via `Container` `:111` |
| `/dashboard` | 0 | 0 | 0 | `dashboard/+page.svelte:138`; popover `HelpTip` `:253` behind an `{#if}` — not in the DOM when closed |

It can distinguish 0 from 1: `/dashboard` expects 0 while `/attendance` expects 1 on the same assertion. Both named reddening mutations genuinely redden — dropping the `{#if description}` guard makes `/settings/roles`, `/leave/balances` and `/dashboard` each gain an `About {title}` button (0 → 1); adding a description to `<PageHeader title="Dashboard" />` at `:138` takes `/dashboard`'s total 0 → 1.

**FIX-3 (was FAIL-3) — the unit gate predicate. CONFIRMED FIXED, and confirmed by execution.** The exact predicate from Step 3 (`/<PageHeader\b[^>]*>/g`, `description=` tested only against those tags, `{#snippet badge()}` body tested for `HelpTip`) was run over the untouched working tree:

- Untouched tree → `offenders === []` → **GREEN on arrival**. ✓
- Old file-wide `/description=/` predicate on the same tree → `offenders === ["src/routes/(app)/leave/balances/+page.svelte"]` → **red on arrival**, exactly as the plan states. The cause is confirmed: `leave/balances/+page.svelte:148` carries `description={filtered ? … : undefined}` on an `<EmptyState>`.
- With the named mutation applied in memory (`description="Remaining days per employee."` added to `<PageHeader title="Leave Balances">` at `:31`) → `offenders === ["src/routes/(app)/leave/balances/+page.svelte"]` → **RED**. ✓

The gate can pass and can fail. It is a guard, not a tautology, and it is not red on arrival.

### Dimension findings

- Infra fit: **PASS** — `lucide-svelte@^0.460.0` is a dependency (`package.json:75`) and `calendar-days.svelte`, `table-2.svelte`, `grid-3x3.svelte` all exist under `node_modules/lucide-svelte/dist/icons/`. `paginate(url, total, { param, pageSize })` matches `src/lib/server/pagination.ts:38-65`; the fetch-then-slice precedent is real at `separations/+page.server.ts:29` and `inventory/+page.server.ts:40`, and the `myPage`/`teamPage` precedent at `timesheets/+page.server.ts:62,79`. `vitest.config.ts` is `environment: 'node'`, `include: tests/unit/**` — the source-assertion approach is the only one this repo supports. `Pagination.svelte:28` renders nothing at `total <= pageSize`, so N6-AC9 needs no conditional. No step runs `./start.sh`, vite or docker; the e2e webServer is Playwright's own `build && preview` lifecycle, unchanged.
- Test coverage: **CONCERN** — see the anti-flake gap below. Every other gate is real, targeted, and executable; no criterion rests on Known-Gap.
- Breaking changes: **PASS** — `listAssignableEmployees(organizationId)` keeps its one-argument signature (`src/lib/server/services/settings/org.ts:394`); filtering and slicing happen in the load. `PageHeader`'s prop list is unchanged (`PageHeader.svelte:8-22`). `?view=` values are untouched. No step stages `src/lib/rbac.ts`, `prisma/schema.prisma` or `src/lib/server/services/**` — verified against all four commit stagings.
- Security surface: **PASS** — no auth, billing, schema, migration, secret or trust-boundary surface. `requireAnyCapability(user.roles, 'MANAGE_HR')` stays first in the rewritten load.
- Section N7 feasibility: **PASS** — the `PageHeader.svelte:33-42` anchor is verbatim-exact. `HelpTip.svelte:9` is `class="group inline-flex"` with no `relative`, and its tooltip span at `:18-24` is `absolute left-0 top-full`, so the `relative` div at `PageHeader.svelte:35` really is the only positioned ancestor — Step 2's placement is load-bearing and correct. `HelpTip.svelte:4` declares `label: string` required with no default and `children: Snippet`.
- Section N1 feasibility: **PASS** — anchors verbatim at `AttendanceHrGrid.svelte:207-238`, `240-246`, `258-259`, `318-323`. `/attendance` defaults to `view=matrix` for a `canManage` user (`attendance/+page.server.ts:147-150`), so `TeamMatrix` renders under `{#if data.view === 'matrix' && data.matrix}` at `:579`. `'Whole team (day)'` appears **0** times in `src/` — the old assertion at `employee-view-only.spec.ts:170` was provably vacuous.
- Section N4 feasibility: **PASS** — anchor verbatim at `separations/+page.svelte:22-35`; `SeparationCreateDialog bind:open={showForm}` at `:37` is untouched by the move.
- Section N6 feasibility: **CONCERN** — load and svelte anchors are verbatim-correct; two line-range labels drift (below). The anti-flake gap is the substantive item.

### Concerns re-checked from the brief

- **CONCERN-2 fix — CONFIRMED.** `grep -c "New Separation" tests/e2e/separations.spec.ts` returns **0**. `--list` shows the spec's 6 tests are the capability gate and the finalize/undo flow; none touches the button or the dialog. The new `the button still opens the create dialog` step is therefore the only thing that proves N4-AC3, and the demotion of the existing spec to a regression check is correct.
- **CONCERN-3 fix — CONFIRMED with one remaining gap.** The FK really is RESTRICT: `PayrollEntry.employee @relation(fields: [employeeId], references: [id])` in `prisma/schema.prisma` declares no `onDelete`, and Prisma's default for a required relation is `Restrict`. The ordering note is correct and already documented verbatim at `tests/e2e/pagination.spec.ts:69-71`. `test.describe.configure({ mode: 'serial' })` is at `pagination.spec.ts:10`; `SURNAME='Zzpagetest'` / `COUNT=25` at `:12-13`; the best-effort `afterAll` at `:67-79`. The seed check also holds: `listAssignableEmployees` filters only on `organizationId` (`org.ts:394-408`) and `prisma/seed-core.ts` contains 15 `firstName:` entries across all three tenants, so the shared seed is well under 21 — the spec must seed its own fixtures. **Gap:** serial mode does not stop cross-*file* parallelism. `playwright.config.ts:20` sets `fullyParallel: true` and `:23` sets `workers: process.env.CI ? 1 : undefined`, so locally `pagination.spec.ts` can inject its 25 `Zzpagetest` rows into the same Veent org while `settings-org-assignments.spec.ts` walks pages. Requirement 1's stated protection is wrong; only requirement 2 (read M from the page) and CI's `workers: 1` actually protect. The unfiltered multi-page walks (`table is bounded`, `counter tells the truth`, `filter applies across pages`) stay exposed on a local run.
- **CONCERN-4 — CONFIRMED, and correctly handled.** The native `×` on `<input type="search">` fires `input` (and, in Chromium, `search`) but never `submit`, so clearing the box that way will not restore the unfiltered rows until Enter or `Filter`. P-D3 records it, forbids the `onsearch` workaround by name because it would reintroduce per-keystroke navigation, and routes it to owner sign-off. It is flagged, not papered over.

  **RESOLVED 18-09-26 — D18.** The Employee Assignments filter row gets a `Clear` link, matching the one `/inventory` already renders (`inventory/+page.svelte:43-81`). It is a plain link to the route with no filter params, so it resets the search text AND the "only unassigned" checkbox in one action, and it works in every browser — Firefox does not draw the native `×` at all, so a `search`-event handler could never have been the whole answer. The `onsearch` workaround stays refused: it would reintroduce per-keystroke navigation for a gain that only two browsers see. The native `×` remains inert by design; the `Clear` link is the supported way to clear. Add an acceptance check to the N6 spec: activating `Clear` restores the unfiltered first page and unticks the checkbox.
- **CONCERN-9 — CONFIRMED.** N7's own spec uses `/separations` as its reference page and asserts title-row geometry at three widths. Today that `PageHeader` is a bare child of `div.space-y-6` (`separations/+page.svelte:22-26`); after N4 Step 13 it sits inside `min-w-0 flex-1` in a `justify-between` row beside the button. A lane-local green on `page-header-helptip` before N4 lands does not carry, and only the full `bun run test:e2e` after Commit 4 catches it. The plan says exactly this. One named fallback is wrong: `/complaints` as an HR user already renders a title-row `{@render actions()}` cluster (`complaints/+page.svelte:77-87`), so it is not a control-free reference page. `/inventory` is (`inventory/+page.svelte:33-36`, bare child of `space-y-6`).
- **The rejected finding (prior CONCERN-10) — the PLAN IS RIGHT; the rejection was the right call.** Read directly from source: `src/lib/components/attendance/AttendanceHrGrid.svelte:258` is `</form>` and `:259` is `{:else if data.view === 'employee'}`. The prior claim that `{:else if}` sits at `:258` does not reproduce. Recording it as not-reproducing was correct.

### Fact corrections re-checked

- **`Pagination.svelte` importers: 19 — CONFIRMED.** `grep -rln` returns 20 paths, but `src/lib/components/employees/employee-tabs.ts:22` only names the file inside a prose comment; it is not an import. 19 real importers stand.
- **"16 routes" is wrong — it is 17.** 15 route pages, plus `/attendance` (via `AttendanceHrGrid`, `AttendanceSelfView`, `TeamMatrix`), plus `/timesheets` (via `TimesheetListTab`, used only by `timesheets/+page.svelte`). Both the plan and the existing backlog note carry the 16.
- **The two `<span>` boundary controls — CONFIRMED.** `Pagination.svelte:40` renders `← Previous` as a `<span>` when `meta.page === 1`, and `:55` renders `Next →` as a `<span>` on the last page. Neither is an anchor, so `data-sveltekit-keepfocus` on the links at `:33`/`:48` genuinely cannot finish the job.

### Open gaps

1. **N6 e2e cross-file fixture race (CONCERN — execute-agent instruction).** Add prefix isolation to the unfiltered multi-page walks: search the `Zzorgtest` marker first, then page, mirroring what `pagination.spec.ts` already does with its surname filter. Alternatively run the full suite with `CI=1` so `workers: 1` applies. Serial mode alone does not close this.
2. **Backlog-note duplication (CONCERN — plan text fix).** The plan mandates writing `process/features/ui-ux-overhaul/backlog/pagination-focus-after-page-change_NOTE_18-09-26.md` at EXECUTE and blocks VERIFIED until it exists. **That note already exists under a different name** — `process/features/ui-ux-overhaul/backlog/pagination-drops-keyboard-focus_NOTE_18-09-26.md`, dated 18-09-26 — and already carries every fact the plan asks for, including the 19-files figure and the `:40`/`:55` span caveat. Following the plan as written creates a duplicate artifact. Retarget the plan (and the Phase Completion Rules clause) to the existing path; write nothing new.
3. **A second unsanctioned code comment (CONCERN — plan text fix).** The plan sanctions exactly one new comment (the N6 load comment). Step 3's unit-test body carries a second explanatory comment — the two-line `// Scope to the <PageHeader ...> OPENING TAG. …` block. Either delete it from the test body (the rationale already lives in the plan prose) or extend the sanction to name it. As written the plan contradicts itself. The `// eslint-disable-next-line` in Step 19 is a directive, not narration, and is fine.
4. **Line-range labels that drift from source (CONCERN — cosmetic).** Four: the `{#if openPanel === 'regularizations'}` guard is at `dashboard/+page.svelte:244`, not `:245`; the `settings/org/+page.svelte` filter-state block is at `34-46`, not `32-46`; the `badge-class-literals.test.ts` `sourceFiles` walker is at `:35-41`, not `:34-40`; `pagination.spec.ts`'s `SURNAME`/`COUNT` are at `:12-13`, not `:13-14`. Every anchor is quoted verbatim, so string-matching at EXECUTE is unaffected. The plan's "33 descriptions across 30 files" is also off — it is 33 across **31** files — and "~52 routes" understates the `PageHeader` reach (62 call sites).
5. **N7-AC7's zero counts depend on `exact: true` being case-sensitive.** `About Leave Balances` vs `About leave balances`, and `About Roles & Permissions` vs `About roles and permissions`, differ only in case and wording. Playwright's `exact: true` is case-sensitive and whole-string, so the zeros are correct. It fails safe if someone drops the flag (count becomes 1 where 0 is expected → red), but the dependency should be stated in the spec.
6. **`/attendance`'s total of 2 holds only on `view=matrix`.** `TeamMatrix` renders under `{#if data.view === 'matrix' && data.matrix}` (`AttendanceHrGrid.svelte:579`), which is the default for a `canManage` user. On `?view=team` the total is 1. Pin the spec to the bare `/attendance` URL or state the view.
7. **`e2e-spec-filter-silently-ignored_NOTE_10-09-26.md` is stale** (pnpm-era; the bun scripts filter correctly). Out of this lane's scope — flag for UPDATE PROCESS, do not fix here.
8. **N6-AC10 page-change focus — DEFERRED by explicit owner decision**, not a gap opened here. Confirmed the plan does not smuggle the shared-component edit back in: `Pagination.svelte` appears in neither Touchpoints, Blast Radius, nor any of the four `git add` lines. Only `<Pagination meta={data.employeePagination} />` is added at a call site.

### What this coverage does NOT prove

- `bun run test -- page-header-helptip` is a **source assertion**. It proves the `PageHeader` source text and the repo-wide call-site invariant. It proves nothing about rendered output, computed styles, tooltip position, or that `HelpTip` receives the string at runtime — only the e2e spec touches those.
- `bun run test:e2e -- page-header-helptip` proves the four pages in its expected-count table. It does not prove the other 27 description-carrying files render one `?` each, nor that any of the 33 description strings is still legible at its own page's width.
- The `anchoring and overflow at three widths` test proves the tooltip does not spill at 1280/768/375 on the `/separations` title row **after N4 has landed**. It does not prove anchoring on a page with a long title, a badge pill, and a Back button all in the same cluster — no page in the expected-count table has that combination.
...
- `bun run test:e2e -- settings-org-assignments` proves paging and filtering against whatever roster exists at run time. It does not prove behaviour at real tenant scale, nor that the unbounded `listAssignableEmployees` query stays acceptable, nor — until gap 1 is closed — that a concurrent `pagination.spec.ts` has not changed the roster mid-walk.
...
- Nothing here proves the four commits are free of the repo's known e2e flake (#287). Only a recorded pre-change baseline makes a post-change count meaningful.
- The contrast probe (N7-AC8) and the three focus-ring clauses (N1-AC8, N4-AC5, N6-AC10) are judgment gates. A green suite does not carry them; the owner's light/dark pass does.

Gate: CONDITIONAL (0 FAILs; 8 concerns recorded — 3 are plan-text fixes, 1 is an execute-agent instruction, 4 are cosmetic or out-of-scope; the one deferred clause is owner-decided and already has its backlog artifact on disk)
Accepted by: session (autonomous, outer-PVL) — accepted concerns, by name: N6 e2e cross-file fixture race; backlog-note duplication; second unsanctioned code comment; drifting line-range labels; `exact: true` case-sensitivity dependency; `/attendance` total is view-dependent; stale `e2e-spec-filter-silently-ignored` note; N6-AC10 page-change focus (owner-deferred, artifact exists).

**Before Commit 1, apply the three plan-text fixes (gaps 1–3).** They are edits to this plan file only, no source change, and each is one line: retarget the backlog note path to `pagination-drops-keyboard-focus_NOTE_18-09-26.md`; remove or sanction the test-body comment; add the prefix-isolation instruction to the N6 e2e spec.

**Autonomous goal block:** not written to this plan. An umbrella plan with a `## Stable Program Goal` exists at `process/features/ui-ux-overhaul/active/ui-ux-overhaul_03-09-26/ui-ux-overhaul-umbrella_PLAN_03-09-26.md` — the umbrella's `/goal` governs (BRANCH B).
