...
{:then list}
	...
	{@const cols = (showEmployee ? 4 : 3) + (canModify ? 1 : 0)}
		...
		<table class="w-full min-w-[44rem] table-fixed text-sm">
			<thead class="border-b bg-muted/50">
				<tr>
					...
					<th class="w-40 px-4 py-3 text-right font-medium text-muted-foreground whitespace-nowrap"
						>Total Hours</th
					>
			...
			<tbody class="divide-y">
				{#each list as ts (ts.id)}
					<tr
						onclick={() => onopen(ts)}
						onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && onopen(ts)}
						tabindex="0"
					...
					>
						...
						>
						<td class="px-4 py-3 text-right tabular-nums">{Number(ts.totalHours).toFixed(2)} hrs</td
						>
