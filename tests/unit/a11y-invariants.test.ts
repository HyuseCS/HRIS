...
describe('table rows are rows, not fake links (S4 items 22-25)', () => {
	...
	it('the timesheets row opens its modal from a real button, not from the row', () => {
		...
		// The list markup now lives in the shared tab component, not in the route.
		const page = read('lib/components/timesheets/TimesheetListTab.svelte')
	...
	it('no converted row is still a focusable fake control', () => {
		...
		expect(read('lib/components/timesheets/TimesheetListTab.svelte')).not.toContain('tabindex="0"')
	})
})
...
describe('icon, tab and error affordances (S6 items 35-37)', () => {
	...
	it('the five long surfaces take a failed submit to the error', () => {
		// Addendum §F. A page that dropped its mechanism would leave HR staring at an unchanged
		// screen after a failed save far below the fold. Three surfaces scroll the message into
		// view; the other two reached the same end by a different route before this phase landed,
		// so each is pinned on the mechanism it actually uses.
		const SCROLLED = [
			// The attendance route is a shell — the import card and its error live in the HR grid.
		...
		for (const page of SCROLLED) {
			expect(read(page), page).toContain('use:scrollToError')
		}
		// Approvals routes every action outcome through a viewport-fixed toast, so there is no
		// off-screen message left to scroll to.
		expect(read('routes/(app)/requests/approvals/+page.svelte')).toContain('submitFeedback')
		// Roles refuses inside its editor dialog and pulls focus onto the refusal itself.
		expect(read('routes/(app)/settings/roles/+page.svelte')).toContain('errorEl?.focus()')
	})
