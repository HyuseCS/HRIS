...
describe('the Stores/Team noun ruling reaches non-nav surfaces (S2 item 14)', () => {
	...
	/**
	 * The multi-day attendance matrix moved out of `/team` into `/attendance` (17-09-26
	 * restructure), so the heading half of the ruling is pinned where the heading now lives.
	 */
	it('the attendance matrix heading is Team Attendance for every tenant', () => {
		const matrix = read('lib/components/attendance/TeamMatrix.svelte')
		expect(matrix).toContain('>Team Attendance</h2>')
		expect(matrix).not.toContain('isFoodService')
	})
})
