> # ⛔ OVERRIDE — THIS HARNESS IS DORMANT ON VEENT HRIS
>
> **Everything below this block is OFF BY DEFAULT.** It is reference material, not
> standing instructions. On this repo the RIPER-5 / vc-harness is **opt-in**, matching
> how it is installed on `parasat` and the opposite of `veent-wifiportal`, where it is
> mandatory — do not carry that repo's rule across.
>
> **Activate only when the user names it in that same turn** — "use the harness",
> "RIPER-5", "vc-riper 5", or a direct `vc-*` invocation. Activation does **not**
> persist: the next turn is dormant again unless it, too, names the harness.
>
> **Never offer or suggest the harness.** Not for large work, not for migrations,
> not as an option among others. The user asks or it does not run.
>
> **Two mechanisms keep it passive, and both must stay intact:**
> 1. There is **no `.claude/settings.json`** in this repo. That file is what wires the
>    hooks in `.claude/hooks/`; its absence is the enforcement. Do not create or
>    restore it unasked — not even to "fix" a hook that appears broken.
> 2. This block. Do not remove or weaken it when editing the rest of the file.
>
> Ordinary work on this repo follows `~/.claude/CLAUDE.md` and normal Claude Code
> behaviour. The mode agents, phase programs and plan lifecycle below apply **only**
> inside an explicitly activated turn.
>
> Unlike `parasat`, `CLAUDE.md` here is **not** a symlink to this file — it is the
> project's own instructions (tech stack, Prisma enum traps, commit rules) and stays
> authoritative for ordinary work. This file is the harness entry point only.
